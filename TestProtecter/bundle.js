...
$("#btn1").click(function () {
                ...
                if(parseInt(http_count)>0){
                    ...
                    xhr1.onreadystatechange = function(){
                    if(xhr1.readyState == 4){
                        http_list = xhr1.responseText
                        console.log(http_list)
                        chrome.tabs.sendMessage(honey_tag_id,{http_list:http_list} , function (res) {
                        if (res) {
                            console.log(res);
                            cb(null, res);
                        }
                ...
                if(parseInt(msg_count)>0){
                    ...
                    xhr.onreadystatechange = function(){
                        if(xhr.readyState == 4){
                            msg_list = xhr.responseText
                            console.log(msg_list)
                            chrome.tabs.sendMessage(honey_tag_id,{msg_list:msg_list} , function (res) {
                            if (res) {
                                console.log(res);
                                cb(null, res);
                            }
                ...
                }else if(parseInt(msg_count)==0){localStorage.setItem('mlist',0)}
...
$("#btn2").click(function () {
        ...
        var status = document.getElementById('p_honey_id');
		status.innerText="on ready";
        chrome.tabs.reload(honey_tag_id, { bypassCache: true },
                    function () {
                        var _log = getlog('Honeysite reloaded');
                        console.log(_log, 'color:pink');
...
function getProfile(ext_list){
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "http://localhost:3000/get_profile");
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    post = '';
    count =0;
    temp = ''
...
function Run(){
    async.waterfall([
        function(cb){
            chrome.management.getAll(async function(e){
                ...
                xhr.onreadystatechange = function(){
                if(xhr.readyState == 4){
                    global_dict =xhr.responseText;
                    threat_list = Object.keys(global_dict);
                    ext_list.forEach((a)=>{
                        let c = threat_list.findIndex(b=>a ===b);
                        if(c>-1) delete threat_list[c];
                        else rest_list.push(a);
                    })
                    cb(null,{'threat':threat_list, 'rest':rest_list,'idlist':idlist})
                }
               }
               })},
               function(dic,cb){
                   ...
                   var threat = dic['threat']
                   var rest = dic['rest']
                   var total = dic['idlist']
                   for(var index=0;index<total.length;index++){
                        if(threat.indexOf(total[index].id)>-1){
                         _threat.push(total[index])
                        }
                        else if(rest.indexOf(total[index].id)>-1){
                            _rest.push(total[index])
                        }
                   }
                   var items = document.createElement('div')

                   for(k=0;k<_threat.length;k++){
                        var temp = _threat[k];
                        items.appendChild(getAlertItemDiv(temp))
                   }
                   for(x=0;x<_rest.length;x++){
                        var _temp = _rest[x]
                        items.appendChild(getItemDiv(_temp))
                   }
                   cb(null,items);
        ...
        function(tags, cb){
            console.log(tags);
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "http://localhost:3000/generate_noise");
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            var jtags= JSON.parse(tags);
            var post_2 = ''
            for(var x=0; x<Object.keys(jtags).length; x++){
                temp = Object.keys(jtags)[x] + '='+ Object.values(jtags)[x]+'&'
                console.log(temp)
                post_2 += temp
            }
            post_2 = post_2.substring(0,post_2.length-1)
            console.log(post_2)
            xhr.send(post_2)
            xhr.onreadystatechange = function(){
                console.log(xhr.readyState);
        ...
        function(fin,cb){
            ...
            chrome.tabs.query({active: true, currentWindow: true}, function(tabs){
                var id = tabs[0].id;
                    chrome.tabs.sendMessage(id, fin, function(response) {
              ...
              console.log(honey_tag_id);
               chrome.tabs.sendMessage(honey_tag_id,{finset:fin} , function (res) {
                    if (res) {
                        console.log(res);
                        cb(null, res);
                    }
   ...
   /* async.waterfall([
        ...
        function(list, cb){
            ...
            console.log(honey_tag_id);
            chrome.tabs.sendMessage(honey_tag_id,{happy:''} , function (res) {
                    if (res) {
                        console.log(res);
                        cb(null, res);
                    }
